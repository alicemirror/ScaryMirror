/**
 * @file  ScaryMirror_ArduinoLight
 * @brief Neopixel functions on the Arduino controlled by the Raspberry PI through
 * the I2C protocol.
 * 
 * The I2C protocol is used to control the lighting functions by the Raspberry PI 
 * so the Arduino is set as slave at the I2C_ADDR address. Due the reduced lenght
 * of the wiring (about 20 cm) and the PI3B+ internal pull-up resistors on the two
 * I2C protocol PINs no extra pull-up external resistors are needed.
 * 
...
//! Undef to remove serial debug messages
#define _DEBUG
...
/**
    ...
    @note When the Wire (I2C) is initialiazed at the address I2C_ADDR the library
    protocol is also connected to the receiveEvent() callback function. Everytime
    a command is sent from the Raspberry PI through the I2C protocol the Arduino slave
    receive it and all the function that process the content (a numeric value) corresponding
    to a light function command.
 */
...
void loop() {
    ...
    colorWipe(FIRE1, 10); colorWipe(FIRE2, 10); colorWipe(FIRE3, 10);
    colorWipe(FIRE4, 10); colorWipe(FIRE5, 10); colorWipe(FIRE6, 10);
...
/**
 * Light testing function. It is not used, call it only
 * for testing purposes
 */
void testLights() {
    // Rotate the strip with one color on and off 
    colorWipeOnOff(FIRE1, 10); colorWipeOnOff(FIRE2, 10); colorWipeOnOff(FIRE3, 10);
    // Rotate the strip with one color
    colorWipe(FIRE1, 10); colorWipe(FIRE2, 10); colorWipe(FIRE3, 10);
    colorWipe(FIRE4, 10); colorWipe(FIRE5, 10); colorWipe(FIRE6, 10);
    // Flash colors
    colorFlash(FIRE1, 10); colorFlash(FIRE2, 10); colorFlash(FIRE3, 10);
    colorFlash(FIRE4, 10); colorFlash(FIRE5, 10); colorFlash(FIRE6, 10);
    // Fixed colors
    setColor(FIRE1); delay(1000); setColor(FIRE2); delay(1000);
    setColor(FIRE3); delay(1000); setColor(FIRE4); delay(1000);
    setColor(FIRE5); delay(1000); setColor(FIRE6); delay(1000);
    setColor(BLUE1); delay(500); setColor(BLUE2); delay(500);
    setColor(BLUE3); delay(500); setColor(BLUE4); delay(500);
    setColor(PURPLE1); delay(500); setColor(PURPLE2); delay(500);
    setColor(PURPLE1); delay(500); setColor(PURPLE4); delay(500);
}


/**
 * Intercept function executed whenever the I2C master send data to 
 * this slave address. In the setup() initialiazation this function is
 * registered as an event 
 * 
 * @note Received commands arre xpected in the form "Cn" where "n" is
 * the command ID
 */
void receiveEvent(int howMany) {

    // Loop until remain one byte in the I2C serial buffer
